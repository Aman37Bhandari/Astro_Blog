...
export default function BlackHole() {
  ...
  useEffect(() => {
    ...
    const DISK_OUTER_RADIUS = 7.4;
    const DISK_TILT_ANGLE = Math.PI / 2.5;
    ...
    // --- Composer with Alpha Channel Support ---
    const parameters = {
    ...
    // Modified lensing shader to preserve alpha
    const lensingShader = {
      uniforms: {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec4 texelR = texture2D(tDiffuse, distortedUvR);
          vec4 texelG = texture2D(tDiffuse, distortedUvG);
          vec4 texelB = texture2D(tDiffuse, distortedUvB);
          
          // Preserve alpha channel
          float alpha = (texelR.a + texelG.a + texelB.a) / 3.0;
          
          gl_FragColor = vec4(texelR.r, texelG.g, texelB.b, alpha);
        }
      `
    };
    
    const lensingPass = new ShaderPass(lensingShader as any);
    lensingPass.clear = false;
    ...
    // --- Black Core ---
    const blackHoleGeom = new THREE.SphereGeometry(BLACK_HOLE_RADIUS*0.5, 128, 64);
    ...
    const diskMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uColorMid1: { value: new THREE.Color(0xffaa33) },
        uColorMid2: { value: new THREE.Color(0xff5522) },
        uColorMid3: { value: new THREE.Color(0xff2200) },
        uColorOuter: { value: new THREE.Color(0xaa3300) },
    ...
    function animate() {
      ...
      // Compute screen position for lensing
